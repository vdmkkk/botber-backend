...
from app.schemas.auth import RegisterIn, VerifyEmailIn, LoginIn, TokenPair, ForgotPasswordIn, ResetPasswordIn, ChangePasswordIn
    ...
    if ver:
        await db.execute(
            ...
            .values(
                ...
                telegram=data.telegram, password_hash=hashed, code=code, expires_at=expires_at,
                created_at=datetime.now(timezone.utc),
            )
        )
    else:
        ver = EmailVerification(
            email=data.email, name=data.name, surname=data.surname, phone=data.phone,
            telegram=data.telegram, password_hash=hashed, code=code, expires_at=expires_at,
            created_at=datetime.now(timezone.utc),
        )
        db.add(ver)
...
async def verify_email(data: VerifyEmailIn, db: AsyncSession = Depends(get_db)):
    ...
    # create user
    user = User(
        name=ver.name, surname=ver.surname, email=ver.email,
        phone=ver.phone, telegram=ver.telegram, password_hash=ver.password_hash,
    )
    db.add(user)
    await db.commit()
...
async def login(data: LoginIn, db: AsyncSession = Depends(get_db)):
    ...
    if await is_blocked(data.email):
        ...
        raise HTTPException(status_code=429, detail=f"Too many attempts. Try again in {ttl or settings.LOGIN_BLOCK_SECONDS}s")
...
async def forgot_password(data: ForgotPasswordIn, db: AsyncSession = Depends(get_db)):
    ...
    pr = PasswordReset(user_id=user.id, token=token, expires_at=expires_at, used=False, created_at=datetime.now(timezone.utc))
    db.add(pr)
    await db.commit()
...
async def change_password(
    ...
    uid = await get_session_user_id(x_session_token) if x_session_token else None
    if not uid:
        raise HTTPException(status_code=401, detail="Invalid session")
    user = await db.get(User, uid)
    if not user:
